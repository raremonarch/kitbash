...
def _make_runner(pkg: MockPkg, shell: MockShell, enabled: list[str], tmp_path: Path) -> Runner:
    modules_dict = {name: True for name in enabled}
...
def test_discover_finds_stub_modules(mock_pkg, mock_shell, tmp_path) -> None:
...
def test_enabled_modules_filtered(mock_pkg, mock_shell, tmp_path) -> None:
...
def test_enabled_modules_sorted_by_tier(mock_pkg, mock_shell, tmp_path) -> None:
...
def test_run_all_installs_enabled(mock_pkg, mock_shell, tmp_path) -> None:
...
def test_run_all_skips_already_installed(mock_pkg, mock_shell, tmp_path) -> None:
...
def test_run_one(mock_pkg, mock_shell, tmp_path) -> None:
    runner = _make_runner(mock_pkg, mock_shell, ["_test_alpha"], tmp_path)
    result = runner.run_one("_test_alpha")
    assert result.status == "success"


def test_run_one_unknown_raises(mock_pkg, mock_shell, tmp_path) -> None:
    runner = _make_runner(mock_pkg, mock_shell, [], tmp_path)
    with pytest.raises(ValueError, match="Unknown module"):
        runner.run_one("_test_does_not_exist")


def test_run_one_unknown_no_modules_message(mock_pkg, mock_shell, tmp_path, monkeypatch) -> None:
    runner = _make_runner(mock_pkg, mock_shell, [], tmp_path)
...
def test_uninstall_all_reverse_order(mock_pkg, mock_shell, tmp_path) -> None:
...
def test_state_recorded_after_run(mock_pkg, mock_shell, tmp_path) -> None:
    runner = _make_runner(mock_pkg, mock_shell, ["_test_alpha"], tmp_path)
